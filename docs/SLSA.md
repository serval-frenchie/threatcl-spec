...
| **4** | `govulncheck` workflow (push/PR + weekly, reachability-aware) | Hygiene — gates known-vuln deps that this code actually reaches |
| **4** | `dependency-review` PR gate | Hygiene — blocks a vulnerable dep the moment a PR introduces it |
| **4** | `zizmor` workflow (SARIF → code scanning) | Hygiene — static analysis of the CI control plane itself |
...
- [x] **Phase 4** — Scanning, CI hardening & SBOMs (mirrors
  [`threatcl/threatcl` phase 5](https://github.com/threatcl/threatcl/pull/179)):
  `govulncheck.yml` (reachability-aware Go vuln scan on push/PR + weekly),
  `dependency-review.yml` (PR gate on newly-introduced vulnerable deps),
  `zizmor.yml` (static analysis of the workflows themselves, SARIF → code
  scanning), `codeql.yml` (Go, `security-extended` suite), harden-runner in
  egress-audit mode + `persist-credentials: false` on every job, and an SPDX
  SBOM of the Go module tree attached to each release (covered by `SHA256SUMS`
  and the provenance attestation). Reachable vulns flagged by the new
  govulncheck gate were fixed by bumping the Go toolchain (`go 1.25.8` →
  `1.25.11` in `go.mod`, which CI installs via `go-version-file` — this clears
  nine reachable stdlib vulns) plus `golang.org/x/image` (v0.43.0),
...
- `spec-X.Y.Z.spdx.json` — an **SPDX SBOM** of the Go module tree (generated by
  syft from `go.mod`/`go.sum`), so CVEs disclosed later can be matched against
  this release without rebuilding it.
- `SHA256SUMS` — SHA-256 checksums of the archive and the SBOM.
...
```bash
# 1. Integrity — the downloaded assets match the published checksums.
#    SHA256SUMS lists both the archive and the SBOM; --ignore-missing lets you
#    verify just the assets you fetched (e.g. the archive alone). Drop it to
#    require every listed asset to be present:
sha256sum -c --ignore-missing SHA256SUMS

# 2. Provenance — the artifacts were built by THIS repo's release workflow,
#    on a GitHub-hosted runner, from the tagged commit:
gh attestation verify spec-X.Y.Z.tar.gz    --repo threatcl/spec
gh attestation verify spec-X.Y.Z.spdx.json --repo threatcl/spec
